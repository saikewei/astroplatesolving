...
#include <errno.h>
#include <assert.h>
#include "astrometry/log.h" //# Modified by Robert Lancaster for the StellarSolver Internal Library for logging
    ...
    int srclin)
{
    ...
    if (qfits_memory_table.ncells > 0)
    {
        ...
        for (i = 0; i < QFITS_MEMORY_MAXPTRS; i++)
        {
            ...
            if (nptrs >= qfits_memory_table.ncells)
                break;
        }
    }
    ...
    int lineno)
{
    ...
    if (ptr == NULL)
        return qfits_memory_malloc(size, filename, lineno);
...
void qfits_memory_status(void)
{
    ...
    if (qfits_memory_table.ncells < 1)
        return;
    debug("#----- memory diagnostics -----\n"); // # Modified by Robert Lancaster for the StellarSolver Internal Library for logging
    ...
    const char *mm_filename)
{
    ...
    for (ii = 0; ii < QFITS_MEMORY_MAXPTRS; ii++)
    {
        if (++pos == QFITS_MEMORY_MAXPTRS)
            pos = 0;
        if (qfits_memory_p_val[pos] == NULL)
            break;
    }
    qfits_mem_debug(
...
static void qfits_memory_dumpcell(
    ...
    FILE *out)
{
    if (pos < 0 || pos >= QFITS_MEMORY_MAXPTRS)
        return;
    if (qfits_memory_p_val[pos] == NULL)
        return;
