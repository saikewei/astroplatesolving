                     ...
                     va_list *ap)
{
    ...
    for (i = 0; i < nc; i++)
    {
        ...
        if (columndata)
        { // # Modified by Robert Lancaster for the StellarSolver Internal Library to correct warning
            if (in_memory(table))
            {
...
{
    return (read_array_into(tab, colname, read_as_type, FALSE, 0, inds, N, dest, stride, 0, NULL) == NULL ? -1 : 0);
}
...
int fitstable_read_column_array_inds_into(const fitstable_t *tab,
                                          ...
                                          const int *inds, int N)
{
    return (read_array_into(tab, colname, read_as_type, TRUE, 0, inds, N, dest, stride, arraysize, NULL) == NULL ? -1 : 0);
}
...
{
    return (read_array_into(tab, colname, read_as_type, FALSE, start, NULL, N, dest, stride, 0, NULL) == NULL ? -1 : 0);
}
...
**/
static fitstable_t *_fitstable_open(const char *fn)
{
    fitstable_t *tab;
    tab = fitstable_new();
    if (!tab)
